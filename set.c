...
/*   Updated: 2021/02/26 10:00:55 by kawish        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
const char	*set_fields(const char *p_fmt, va_list ap, struct fields *fp)
{
	...
	if (fp->padding_char == '0' && fp->is_minus)
		fp->padding_char = ' ';
	if (fp->padding_char == '0' && (fp->precision > -1 && (fp->conv_char == 'd' || fp->conv_char == 'u' || fp->conv_char == 'x' || fp->conv_char == 'X'))) // hier misschien no p bij.
		fp->padding_char = ' ';
	return (p_fmt);
}
